...
**Public Marketing Site (NearBuy — Site vitrine):**
- React + Vite SPA artifact (`artifacts/nearbuy-site`) served at base path `/site/`. Public-facing landing page in French with a brand hero, "comment ça marche" cards and a download CTA. Uses Wouter routing + Tailwind v4 utilities directly (no shadcn theme dependency on the homepage / deletion pages, so the pages render correctly even while the design tokens are still being finalized).
- **Account deletion flow.** A footer link ("Demander la suppression de mon compte") opens `/site/supprimer-compte`, where any NearBuy customer or NearBuy Business seller can submit a deletion request without signing in (mandatory for Play Store / App Store / RGPD compliance). The form collects: full name, account email, account type (`customer` / `seller` / `both`), optional reason (≤2000 chars) and an explicit confirmation checkbox. Submission posts to `POST /api/public/account-deletion-requests` (no auth, IP-based sliding-window rate limit of 5 requests / hour). Requests are persisted in the new Mongoose model `AccountDeletionRequest` with `status: pending|in_progress|completed|rejected`, the originating IP and user-agent. After a successful submit the user is redirected to `/site/supprimer-compte/confirmation` which states the 30-day SLA and gives the support email.
