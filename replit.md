...
- **Shared cold-start splash (`AnimatedSplash.tsx` + `.css` in both `nearbuy-admin` and `nearbuy-site`):** A pixel- and timing-faithful CSS port of the mobile Reanimated splash (`artifacts/nearbuy/components/AnimatedSplash.tsx`). Six-step choreography over ~6.2 s with `prefers-reduced-motion` snap-to-final fallback:
  1. 0–700 ms: orange teardrop pin drops in from above (spring-like cubic-bezier)
  2. 900–1700 ms: green storefront (scalloped awning + stripes + navy body + door + green window + shopping bag with handle) materialises inside the pin's white window
  3. 1900–2500 ms: three green sparks burst above-left (bouncy 1.2× → 1× scale)
  4. 2700–3400 ms: navy oval base/shadow appears under the pin tip
  5. 3800–4520 ms: full pin composition shrinks to 0.5× and slides up by 100 px
  6. 4200–4800 ms: "Biz Ici" wordmark + tagline fade in at the top (top: 18 %)
  7. 5700–6200 ms: full-screen fade-out, then `onFinish()`
  Both apps use BEM class prefix `bsplash` and `220 × 280 px` pin viewport to match mobile. Per-app overrides: admin tagline = "Espace administrateur", site tagline = "Trouvez tout près de vous". Splash flag stored in `sessionStorage` (`nearbuy_admin_splash_played` / `bizici_site_splash_played`) so it only plays on first visit per session. Mounted in `Login.tsx` (admin) / `App.tsx` (site).
- **Admin web (`nearbuy-admin`):** Tailwind v4 HSL tokens in `src/index.css` (light + dark) target the BizIci palette (primary 26 91% 54%, sidebar navy 225 55% 23%). Sidebar header in `AdminLayout.tsx` uses `<img src="/bizici-pin.png">` + bicolor wordmark + "Admin" subtitle. The login card replaced the legacy "N" monogram badge with the same `bizici-pin.png`. Browser tab title is "BizIci Admin"; favicon is the BizIci pin SVG. The 404 page uses semantic `bg-background`/`text-foreground`/`text-destructive` tokens.
