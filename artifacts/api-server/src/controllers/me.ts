...
function serializeBasket(
  doc: {
    items: { _id: Types.ObjectId; query: string; addedAt: Date }[];
  } | null,
...
export const meController = {
   ...
   */
  getKarma: async (req: Request, res: Response) => {
  ...
  registerPushToken: async (req: Request, res: Response) => {
    ...
    if (!platform || !PUSH_PLATFORMS.includes(platform as PushPlatform)) {
      res.status(400).json({
  ...
  /**
   * Legal consent audit trail. Each signup screen gates account creation
   * behind an "I accept Terms + Privacy" checkbox. After the Clerk session
   * is active the client POSTs here so we have a server-stamped,
   * tamper-resistant record of *which version* of the legal corpus was
   * accepted, by which signup pathway, and at what time. CNIL/GDPR-friendly.
   *
  ...
  recordConsent: async (req: Request, res: Response) => {
    ...
    if (version.length > MAX_CONSENT_VERSION_LEN) {
      res.status(400).json({
        error: `version is too long (max ${MAX_CONSENT_VERSION_LEN})`,
      });
      return;
    }
  ...
  // ---- Course basket ----------------------------------------------------
  // "Course" = a bundle of free-text product queries the customer plans to
  // look for in one shopping run. Saved as a singleton document per user.
  // Starting a course resolves each query against the live product catalog
  // and returns the nearest in-radius shop carrying a matching product,
  // ordered by distance.
  ...
  addBasketItem: async (req: Request, res: Response) => {
    const raw =
  ...
  removeBasketItem: async (req: Request, res: Response) => {
  ...
  clearBasket: async (req: Request, res: Response) => {
    const userOid = new Types.ObjectId(req.userId);
    await Basket.updateOne(
      { userId: userOid },
      { $set: { items: [] } },
