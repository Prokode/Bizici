...
import { getShopAccess } from "../lib/auth";
import {
...
export const appointmentsController = {
  ...
  create: async (req: Request, res: Response) => {
    ...
    if (shop.kind === "products") {
      res.status(400).json({ error: "This shop does not offer services" });
      return;
    }
    ...
    if (body.serviceId !== undefined && body.serviceId !== null) {
      ...
      const svc = await Service.findOne({
        ...
        .lean();
      if (!svc) {
        res.status(404).json({ error: "Service not found in this shop" });
        return;
      }
      serviceOverrideLoc =
    ...
    if (Number.isNaN(scheduledAt.getTime())) {
      res
        .status(400)
        .json({ error: "scheduledAt must be a valid ISO datetime" });
      return;
    }
    ...
    if (
      ...
      if (effectiveLoc !== "both" && effectiveLoc !== body.serviceLocation) {
        res
          .status(400)
  ...
  accept: async (req: Request, res: Response) => {
    ...
    if (appt.status !== "proposed") {
      res.status(409).json({
        error: `Cannot accept an appointment in status '${appt.status}'`,
      });
      return;
    }
  ...
  decline: async (req: Request, res: Response) => {
    ...
    if (appt.status !== "proposed") {
      res.status(409).json({
        error: `Cannot decline an appointment in status '${appt.status}'`,
      });
      return;
    }
  ...
  complete: async (req: Request, res: Response) => {
    ...
    if (appt.status !== "confirmed") {
      res.status(409).json({
        error: `Cannot complete an appointment in status '${appt.status}'`,
      });
      return;
    }
  ...
  cancel: async (req: Request, res: Response) => {
    ...
    if (appt.status === "cancelled" || appt.status === "declined") {
      res.status(409).json({ error: `Appointment already ${appt.status}` });
      return;
    }
