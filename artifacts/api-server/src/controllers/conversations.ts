...
export const conversationsController = {
  ...
  createOrGet: async (req: Request, res: Response) => {
  ...
  list: async (req: Request, res: Response) => {
    ...
    const sellerShopIds = await ShopMember.find({ userId: userOid })
      ...
      .lean();
    const sellerShopOids = sellerShopIds.map(
    ...
    const convs = await Conversation.find({
      $or: [
        ...
        ...(sellerShopOids.length > 0
          ? [{ shopId: { $in: sellerShopOids } }]
          : []),
      ],
    })
    ...
    const enriched = (
      await Promise.all(
        convs.map(async (c: ConversationDoc) => {
          const role: Role = c.customerUserId.equals(userOid)
            ? "customer"
            : "seller";
      ...
      (x: EnrichedConversation | null): x is EnrichedConversation => x !== null,
    );
  ...
  listMessages: async (req: Request, res: Response) => {
    ...
    if (req.query.before !== undefined) {
      ...
      if (Number.isNaN(parsed.getTime())) {
        res.status(400).json({
          error: "Invalid 'before' query: must be an ISO-8601 datetime",
        });
        return;
      }
      before = parsed;
