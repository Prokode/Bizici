...
export const discountsController = {
  listForProduct: async (req: Request, res: Response) => {
    if (!Types.ObjectId.isValid(String(req.params.productId))) {
      res.status(400).json({ error: "Invalid productId" });
      return;
    }
    const product = await Product.findOne({
      _id: new Types.ObjectId(String(req.params.productId)),
    ...
    const items = await Discount.find({
      product: new Types.ObjectId(String(req.params.productId)),
    })
      .sort({ createdAt: -1 })
      .lean();
  ...
  remove: async (req: Request, res: Response) => {
    if (
      !Types.ObjectId.isValid(String(req.params.discountId)) ||
      !Types.ObjectId.isValid(String(req.params.productId))
    ) {
    ...
    const r = await Discount.deleteOne({
      _id: new Types.ObjectId(String(req.params.discountId)),
      product: new Types.ObjectId(String(req.params.productId)),
    });
