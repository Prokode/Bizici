...
// Mounted on the same prefix as `/api/shops/:shopId/...` and gated by the
// existing seller auth middlewares.
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    // Reject silly payload sizes early. ~7MB base64 ≈ 5MB binary, plenty for an ID.
    const totalSize =
      frontImageBase64.length +
      (typeof backImageBase64 === "string" ? backImageBase64.length : 0);
    if (totalSize > 7_500_000) {
      res.status(413).json({ error: "Image trop volumineuse (max ~5MB / face)." });
      return;
    }
    ...
    await KycDocument.findOneAndUpdate(
      { shopId },
      {
        $set: {
          ...
          submittedAt: now,
          status: "pending",
          // Clear previous review on resubmission.
          reviewedAt: null,
          reviewedBy: null,
  ...
  async (req: Request, res: Response) => {
    ...
    res.json({
      items: shops.map((s) => {
        ...
        return {
          // `id` is required by the admin DataTable component; `shopId`
          // mirrors the value for downstream calls.
          id: sid,
          shopId: sid,
          shopName: s.name,
  ...
  async (req: Request, res: Response) => {
    ...
    await Shop.findByIdAndUpdate(shopId, {
      $set: {
        ...
        "kyc.rejectionReason": null,
        // Mirror legacy provider isVerified flag so older UI bits still light up.
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!reason || reason.length < 5) {
      res.status(400).json({ error: "Une raison de refus claire est requise (≥ 5 caractères)." });
      return;
    }
    const shopId = new Types.ObjectId(req.params.shopId as string);
    const adminId = req.admin?.id ? new Types.ObjectId(req.admin.id) : null;
    ...
    res.json({ ok: true, status: "rejected" as const, rejectionReason: reason });
  },
);
