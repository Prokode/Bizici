...
router.delete("/admin/users/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
    res.status(400).json({ error: "Invalid user id" });
    return;
  ...
  res.json({
    items: items.map((s) => {
      ...
      return {
        ...
        seller: seller
          ? { id: String(seller._id), name: seller.name ?? null, email: seller.email ?? null }
          : null,
      };
    }),
    page,
...
router.get("/admin/shops/:id", async (req: Request, res: Response) => {
  ...
  const [seller, members, productsCount, conversationsCount] = await Promise.all([
...
router.patch("/admin/shops/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
    res.status(400).json({ error: "Invalid shop id" });
    return;
  ...
  if (typeof marketName === "string") shop.marketName = marketName.trim() || null;
...
router.delete("/admin/shops/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
    res.status(400).json({ error: "Invalid shop id" });
    return;
...
router.delete("/admin/products/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
...
router.post("/admin/categories", requireWriter, async (req: Request, res: Response) => {
...
router.patch("/admin/categories/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
...
router.delete("/admin/categories/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
    res.status(400).json({ error: "Invalid category id" });
    return;
  ...
  res.json({
    items: items.map((c) => {
      ...
      return {
        id: String(c._id),
        shop: shop
          ? { id: String(shop._id), name: shop.name, marketName: shop.marketName ?? null }
          : null,
        customer: u
...
router.delete("/admin/invitations/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
...
router.delete("/admin/broadcasts/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
...
router.get("/admin/karma-events", async (req: Request, res: Response) => {
  const page = clampPage(req.query.page);
  const pageSize = clampPageSize(req.query.pageSize);
  const filter: Record<string, unknown> = {};
...
router.delete("/admin/karma-events/:id", requireWriter, async (req: Request, res: Response) => {
  const oid = objectId(req.params.id);
  if (!oid) {
