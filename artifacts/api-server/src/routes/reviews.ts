 ...
 */
router.get("/me/reviews/:shopId", async (req, res) => {
 ...
 * semantics evolve.
 */
router.post("/me/reviews/:shopId", async (req, res) => {
  const shopId = parseShopId(req.params.shopId);
  if (!shopId) {
    res.status(400).json({ error: "invalid shopId" });
    return;
  ...
  if (isMember) {
    res
      .status(403)
      .json({ error: "Sellers cannot review their own shop" });
    return;
  }

  // Confirm the shop exists before we create dangling reviews.
 ...
 * button.
 */
router.delete("/me/reviews/:shopId", async (req, res) => {
  const shopId = parseShopId(req.params.shopId);
  if (!shopId) {
    res.status(400).json({ error: "invalid shopId" });
    return;
