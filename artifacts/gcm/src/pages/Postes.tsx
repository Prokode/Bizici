...
import {
  ...
  Clock4, Coins, Pause, X, Power, Wifi, MessageCircle, Sparkles
} from "lucide-react";
...
const STATUS: Record<Status, { color: string; soft: string; grad: string }> = {
  occupe:      { color: "var(--gcm-fuchsia)",  soft: "var(--gcm-fuchsia-soft)", grad: "linear-gradient(135deg,#EC4899,#C026D3)" },
  libre:       { color: "var(--gcm-emerald)",  soft: "rgba(16,185,129,0.10)",   grad: "linear-gradient(135deg,#34D399,#10B981)" },
  maintenance: { color: "var(--gcm-amber)",    soft: "rgba(245,158,11,0.12)",   grad: "linear-gradient(135deg,#FBBF24,#F59E0B)" },
  reserve:     { color: "var(--gcm-cyan)",     soft: "rgba(6,182,212,0.10)",    grad: "linear-gradient(135deg,#22D3EE,#06B6D4)" },
};
...
const POSTES: Poste[] = [
  ...
  { id: "PC-08",  type: "PC",     status: "reserve" },
  ...
  { id: "SWT-02", type: "SWITCH", status: "reserve" },
    ...
    <button onClick={onClick}
      ...
      <div className="flex items-start justify-between relative">
        <div className="flex items-center gap-2.5">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white shrink-0 gcm-shadow-sm" style={{ background: ty.bg }}>
            <Icon className="w-4 h-4" />
          </div>
          <div>
            <div className="gcm-display text-base font-bold leading-tight">{p.id}</div>
            <div className="text-[10px] mt-0.5 gcm-mono font-semibold" style={{ color: "var(--gcm-text-muted)" }}>{ty.chip}</div>
          </div>
        </div>
      ...
      <div className="mt-3.5 min-h-[64px]">
        ...
        {p.status === "libre" && (
          <div className="flex flex-col items-center justify-center h-16 text-center">
            ...
            <span className="text-xs font-semibold" style={{ color: "var(--gcm-emerald)" }}>{t("postes.available")}</span>
            <span className="text-[10px] mt-0.5" style={{ color: "var(--gcm-text-muted)" }}>{t("postes.tap_start")}</span>
          </div>
        )}
        {p.status === "maintenance" && (
          <div className="flex items-center gap-2 text-xs h-16" style={{ color: "var(--gcm-amber)" }}>
            ...
            <div>
              <div className="font-semibold">{t("postes.out_of_service")}</div>
              <div className="text-[10px]" style={{ color: "var(--gcm-text-muted)" }}>{t("postes.tech_assigned")}</div>
            </div>
          </div>
        )}
        {p.status === "reserve" && (
          <div className="flex items-center gap-2 text-xs h-16" style={{ color: "var(--gcm-cyan)" }}>
            ...
            <div>
              <div className="font-semibold">{t("postes.reserved_at", { time: p.id === "PC-08" ? "20:30" : "21:00" })}</div>
              <div className="text-[10px]" style={{ color: "var(--gcm-text-muted)" }}>{t("postes.people", { n: 2 })}</div>
            </div>
          </div>
        )}
      </div>
      ...
      {isOccupe && (
        <div className="mt-3 pt-3 border-t flex items-center justify-between" style={{ borderColor: "var(--gcm-border)" }}>
          ...
          <div className="gcm-mono text-sm font-bold" style={{ background: s.grad, WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>
            {p.montant}
          </div>
        </div>
      )}
...
function Modal({ poste, onClose }: { poste: Poste; onClose: () => void }) {
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={`${poste.id} session`}
         className="fixed inset-0 z-50 flex items-center justify-center p-6"
         style={{ background: "var(--gcm-modal-backdrop)", backdropFilter: "blur(8px)" }} onClick={onClose}>
      <div className="w-full max-w-md rounded-3xl overflow-hidden gcm-shadow-lg" style={{ background: "var(--gcm-bg-2)" }} onClick={e => e.stopPropagation()}>
        <div className="relative px-6 pt-6 pb-5 text-white overflow-hidden" style={{ background: s.grad }}>
          ...
          <button onClick={onClose} aria-label="Close" autoFocus className="absolute top-4 right-4 w-8 h-8 rounded-lg flex items-center justify-center bg-white/15 hover:bg-white/25 transition">
            <X className="w-4 h-4" />
          </button>
          <div className="relative flex items-center gap-3.5">
            ...
            <div>
              <div className="text-[11px] uppercase tracking-[0.2em] font-semibold opacity-90">{t(`postes.status.${poste.status}`)} · {t("postes.modal.session_live")}</div>
          ...
          <div className="relative mt-4 flex items-center gap-2 text-[11px]">
            ...
            <span className="px-2 py-1 rounded-full bg-white/20 backdrop-blur">{t("postes.modal.ping", { ms: 12 })}</span>
          </div>
        </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl p-3" style={{ background: "var(--gcm-violet-soft)" }}>
              <div className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: "var(--gcm-violet)" }}>{t("postes.modal.start")}</div>
            ...
            <div className="rounded-xl p-3" style={{ background: "var(--gcm-fuchsia-soft)" }}>
              <div className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: "var(--gcm-fuchsia)" }}>{t("postes.modal.duration")}</div>
          ...
          <div className="rounded-2xl p-5 text-center text-white relative overflow-hidden" style={{ background: "var(--gcm-grad-hero)" }}>
            ...
            <div className="relative">
              <div className="text-[10px] uppercase tracking-[0.22em] font-semibold opacity-90">{t("postes.modal.total_to_cash")}</div>
              <div className="gcm-display text-5xl font-bold mt-2 leading-none">{poste.montant}</div>
              <div className="text-[11px] mt-2 gcm-mono opacity-90">{t("postes.modal.rate", { amount: "800 F" })}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <button className="py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5" style={{ background: "var(--gcm-panel-2)", color: "var(--gcm-cyan)" }}>
              <Pause className="w-3.5 h-3.5" /> {t("postes.modal.pause")}
            </button>
            <button className="py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5" style={{ background: "var(--gcm-panel-2)", color: "var(--gcm-amber)" }}>
              <Plus className="w-3.5 h-3.5" /> {t("postes.modal.add_30")}
            </button>
            <button className="py-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5" style={{ background: "var(--gcm-panel-2)", color: "var(--gcm-text-dim)" }}>
              <Power className="w-3.5 h-3.5" /> {t("postes.modal.reboot")}
            </button>
          </div>
          ...
          <button className="w-full py-3.5 rounded-2xl font-bold text-white text-sm flex items-center justify-center gap-2 gcm-shadow-glow gcm-grad-violet hover:scale-[1.01] transition">
            <Coins className="w-4 h-4" /> {t("postes.modal.close_and_cash")}
          </button>
        </div>
      </div>
    </div>
...
function PostesHero({ counts, total }: { counts: Record<Status, number>; total: number }) {
  ...
  return (
    <div className="px-8 pt-7 pb-5 flex items-end justify-between gap-6 flex-wrap" style={{ background: "var(--gcm-bg-2)" }}>
      <div>
        <div className="text-[11px] uppercase tracking-[0.2em] font-semibold mb-1.5" style={{ color: "var(--gcm-violet)" }}>{t("postes.eyebrow_count", { n: total })}</div>
        <h1 className="gcm-display text-[28px] font-bold leading-tight">{t("postes.title")}</h1>
          ...
          {(["occupe","libre","reserve","maintenance"] as Status[]).map(k => {
            ...
            return (
              <div key={k} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold" style={{ background: s.soft, color: s.color }}>
                ...
                {t(`postes.status.${k}`)} · <span className="gcm-mono">{counts[k] || 0}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="flex items-center gap-3">
        <div className="text-right pr-4 border-r" style={{ borderColor: "var(--gcm-border)" }}>
          <div className="text-[10px] uppercase tracking-widest font-semibold" style={{ color: "var(--gcm-text-muted)" }}>{t("postes.occupancy")}</div>
        ...
        <button className="px-3.5 py-2 rounded-xl text-sm font-semibold flex items-center gap-2 border" style={{ borderColor: "var(--gcm-border)", color: "var(--gcm-text-dim)", background: "var(--gcm-panel)" }}>
          <Filter className="w-4 h-4" /> {t("postes.filter")}
        </button>
        <button className="px-4 py-2 rounded-xl text-white text-sm font-semibold flex items-center gap-2 gcm-shadow-glow gcm-grad-violet">
          <Plus className="w-4 h-4" /> {t("postes.new")}
        </button>
      </div>
    </div>
  );
...
export function Postes(props: { dark: boolean; onToggleDark: () => void }) {
  ...
  return (
    <Shell {...props} hero={<PostesHero counts={counts} total={POSTES.length} />}>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-w-[1400px]">
        {POSTES.map(p => <PosteCard key={p.id} p={p} onClick={() => setSelected(p)} />)}
      </div>

      {selected && selected.status === "occupe" && (
        <Modal poste={selected} onClose={() => setSelected(null)} />
      )}
    </Shell>
  );
}
