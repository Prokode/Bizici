...
const NAV: NavItem[] = [
  ...
  { label: "Postes", icon: Monitor, active: true, badge: 24 },
...
export function Shell({ activeLabel, title, subtitle, actions, children }: {
  ...
  return (
    <div className="gcm-root flex" style={{ minHeight: "100vh" }}>
      {/* Sidebar */}
      <aside className="w-64 shrink-0 border-r flex flex-col" style={{ borderColor: "var(--gcm-border)", background: "var(--gcm-bg-2)" }}>
        <div className="px-5 py-5 border-b flex items-center gap-3" style={{ borderColor: "var(--gcm-border)" }}>
          <div className="w-9 h-9 rounded-md flex items-center justify-center gcm-glow-neon" style={{ background: "linear-gradient(135deg, #7CFF6B, #4EE2FF)" }}>
            <span className="gcm-display font-extrabold text-black text-sm">G</span>
          </div>
          <div>
            <div className="gcm-display text-[15px] font-bold gcm-text-glow-neon" style={{ color: "var(--gcm-neon)" }}>GCM</div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: active ? "var(--gcm-neon)" : "var(--gcm-text-dim)",
              ...
              >
                ...
                {it.badge !== undefined && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded gcm-mono"
                    style={{ background: active ? "var(--gcm-neon)" : "var(--gcm-panel-2)", color: active ? "#000" : "var(--gcm-text-dim)" }}>
                    {it.badge}
                  </span>
                )}
              </button>
        ...
        <div className="p-3 border-t" style={{ borderColor: "var(--gcm-border)" }}>
          <div className="flex items-center gap-3 px-2 py-2 rounded-md" style={{ background: "var(--gcm-panel)" }}>
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: "var(--gcm-magenta)", color: "#000" }}>KS</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b flex items-center px-6 gap-4" style={{ borderColor: "var(--gcm-border)", background: "var(--gcm-bg-2)" }}>
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="flex items-center gap-2 text-xs gcm-mono" style={{ color: "var(--gcm-neon)" }}>
            ...
            <button className="relative w-9 h-9 rounded-md flex items-center justify-center border" style={{ borderColor: "var(--gcm-border)", background: "var(--gcm-panel)" }}>
              ...
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center" style={{ background: "var(--gcm-magenta)", color: "#000" }}>5</span>
            </button>
          </div>
        </header>

        {/* Page header */}
        <div className="px-8 pt-6 pb-4 flex items-end justify-between gap-4 border-b" style={{ borderColor: "var(--gcm-border)" }}>
          <div>
